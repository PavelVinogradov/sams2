...
void UrlGroupList::useConnection (DBConn * conn)
{
  if (_conn)
    {
      DEBUG (DEBUG7, "[" << __FUNCTION__ << "] Already using " << _conn);
      return;
    }
  if (conn)
    {
      DEBUG (DEBUG7, "[" << __FUNCTION__ << "] Using external connection " << conn);
      _conn = conn;
  ...
  if (!_conn)
    {
      ...
      DEBUG (DEBUG7, "[" << __FUNCTION__ << "] Using new connection " << _conn);
    }
    else
    {
      DEBUG (DEBUG7, "[" << __FUNCTION__ << "] Using old connection " << _conn);
    }
...
void UrlGroupList::destroy()
{
  if (_connection_owner && _conn)
    {
      DEBUG (DEBUG7, "[" << __FUNCTION__ << "] Destroy connection " << _conn);
      delete _conn;
      _conn = NULL;
    }
  else if (_conn)
    {
      DEBUG (DEBUG7, "[" << __FUNCTION__ << "] Not owner for connection " << _conn);
    }
  else
    {
      DEBUG (DEBUG7, "[" << __FUNCTION__ << "] Not connected");
    }
