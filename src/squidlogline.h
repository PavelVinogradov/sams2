...
protected:
  ...
  IP _ip;                       ///< IP адрес, откуда поступил запрос (третья колонка)
  logCacheResult _cacheResult;  ///< Тип ответа от squid (четвертая колонка, первое поле)
  long _size;                   ///< Размер объекта (пятая колонка)
  string _url;                  ///< URL адрес объекта (шестая колонка)
  string _ident;                ///< Имя авторизовавшегося пользователя или IP адрес, откуда поступил запрос (седьмая колонка)
};
