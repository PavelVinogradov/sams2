...
function URLTimePeriodGB()
{
  ...
  $acount=0;
  $query="SELECT HOUR(time) as hour,MINUTE(time) as minute,SUM(size) as size,SUM(hit) as hit FROM squidlog.cache WHERE cache.user=\"$username\"&&cache.domain=\"$userdomain\"&&cache.date=\"$sdate\"&&HOUR(time)>=$shou&&HOUR(time)<$ehou GROUP BY hour,minute";
...
function userbuttom_4_time($access,$userid)
{
  global $SAMSConf;
