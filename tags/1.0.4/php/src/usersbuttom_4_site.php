...
function UsersSitesPeriod()
{
  ...
  print("<INPUT TYPE=\"HIDDEN\" NAME=\"filename\" id=filename value=\"usersbuttom_4_site.php\">\n");
  NewDateSelect(0,"");
  ...
  $count=1;
  $query="select trim(leading \"http://\" from substring_index(url,'/',3)) as norm_url,sum(size) as url_size,sum(hit) as hit_size from cache where date>=\"$sdate\"&&date<=\"$edate\" group by norm_url order by url_size desc limit 250";
  ...
  while($row=mysql_fetch_array($result))
	 ...
	 if($SAMSConf->realtraffic=="real")
	   ...
	   PrintFormattedSize($row['url_size']);
         ...
         if($SAMSConf->access==2)
           TableCell("<A TARGET=\"BLANK\" HREF=\"main.php?show=exe&function=siteuserslist&filename=usersbuttom_4_site.php&site=$row[norm_url]&SDay=$sday&EDay=$eday&SMon=$smon&EMon=$emon&SYea=$syea&EYea=$eyea&username=$username\" ><FONT COLOR=\"BLACK\">$row[norm_url]</FONT></A>");
         if($SAMSConf->access==1)
