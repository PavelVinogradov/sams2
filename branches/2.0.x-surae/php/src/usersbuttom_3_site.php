...
function UsersSitesPeriod()
{
  ...
  printf("<BR><B>$traffic_2 $bdate $traffic_3 $eddate</B> ");
	...
	$url_size_sum=0;
	$hit_size_sum=0;
	$query="SELECT substring( s_url from position('//' in s_url)+2 for position('/' in substring(s_url from position('/' in s_url)+2 )) ) as url_domain,sum(s_size) as url_size,sum(s_hit) as hit_size  FROM squidcache WHERE s_date>='$sdate'AND s_date<='$edate' AND s_method!='CONNECT' GROUP BY url_domain ORDER BY url_size desc limit 5000";
	$num_rows=$DB->samsdb_query_value($query);
	while($row=$DB->samsdb_fetch_array())
	{
	...
	$query="SELECT substring( s_url from 0 for position(':' in s_url) ) as url_domain,sum(s_size) as url_size,sum(s_hit) as hit_size  FROM squidcache WHERE s_date>='$sdate'AND s_date<='$edate' AND s_method='CONNECT' GROUP BY url_domain ORDER BY url_domain desc limit 5000";
	$num_rows=$DB->samsdb_query_value($query);
	while($row=$DB->samsdb_fetch_array())
	{
		if(strlen($row['url_domain'])>0)
