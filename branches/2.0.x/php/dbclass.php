...
/*
  ...
  $dbname - название базы данных
*/
  function SAMSDB($db, $odbc, $host, $user ,$passwd, $dbname)
  {
    //echo "SAMSDB: $db, $odbc, $host, $user ,$passwd, $dbname\n";
    $this->db_odbc=$odbc;
    $this->db_name=$db;
    ...
    if($this->db_name=="PostgreSQL" && $this->db_odbc==0)
      {
	$link=$this->pgsqldb_connect($host,$user,$passwd,$dbname);
      }
  }
...
function CreateSAMSdb($db, $odbc, $host, $user ,$passwd, $dbname, $create, $muser, $mpass)
{
 $pgdb=array();
...
s_shablonpool bigint default '0', s_userpool bigint default '0', s_auth varchar(4) default 'ip', s_quote int default '100',s_period varchar(3) NOT NULL default 'M', s_clrdate date NOT NULL default '1980-01-01', 
...
//echo "###################################################\n";
//CreateSAMSdbPgSQL("localhost", "postgres", "","samsdb");
//CreateSAMSdbMySQL( "localhost", "root", "","samsdb");
//echo "###################################################\n";
/*
$pgdb[0] = "CREATE TABLE websettings (	s_lang varchar(15) NOT NULL default 'EN', s_iconset varchar(25) NOT NULL default 'classic', s_useraccess smallint NOT NULL default '1', s_urlaccess smallint NOT NULL default '1', s_showutree smallint NOT NULL default '1' , s_showname varchar(5) NOT NULL default 'nick', s_showgraph smallint NOT NULL default '0', 	s_createpdf smallint NOT NULL default '0',	s_version char(5) NOT NULL default '1.0')"; 
$pgdb[1] = "INSERT INTO websettings VALUES('EN','classic','1','1','1','nick','0','0','1.0')";
$pgdb[2] = "CREATE TABLE proxy (  s_proxy_id int NOT NULL AUTO_INCREMENT PRIMARY KEY, s_description varchar(100) default 'Proxy server', s_endvalue bigint NOT NULL default '0', s_redirect_to varchar(100) default 'http://your.ip.address/sams/icon/classic/blank.gif', s_denied_to varchar(100) default 'http://your.ip.address/sams/messages', s_redirector varchar(25) default '0', s_delaypool smallint default '0', s_auth varchar(4) default 'ip', s_wbinfopath varchar(100) default '/usr/bin', s_separator varchar(15) default '+', s_userdomain smallint default '0', s_bigd smallint default '0', s_bigu smallint default '0', s_sleep int default '0', s_parser smallint, s_parser_time int, s_count_clean smallint default '0', s_nameencode smallint default '0', s_realsize varchar(4) default 'real', s_checkdns smallint default '0', s_debuglevel int NOT NULL default '0', s_defaultdomain varchar(25) NOT NULL default 'workgroup', s_squidbase int NOT NULL default '0', s_udscript varchar(25) NOT NULL default 'NONE', s_adminaddr varchar(60), s_kbsize varchar(15) NOT NULL default '1024', s_mbsize varchar(15) NOT NULL default '1048576')";
$pgdb[3] = "INSERT INTO proxy SET s_description='main proxy server' ";
$pgdb[4] = "CREATE TABLE passwd ( s_user varchar(25) PRIMARY KEY, s_pass varchar(60), s_access int default '0', s_autherrorc smallint default '0', s_autherrort varchar(16) default '0' )";
$pgdb[5] = "INSERT INTO passwd VALUES('Admin','00YfpO1MXDzqQ','2','0','' )";
$pgdb[6] = "INSERT INTO passwd VALUES('Auditor','00MTbxknCTtNs','1','0','' )";
$pgdb[7] = "CREATE TABLE shablon ( s_shablon_id int NOT NULL AUTO_INCREMENT PRIMARY KEY, s_name varchar(25), s_shablonpool int default '0', s_userpool int default '0', s_auth varchar(4) default 'ip', s_quote int default '100',s_period varchar(3) NOT NULL default 'M', s_clrdate date NOT NULL default '1980-01-01', s_alldenied int NOT NULL default '0' )";
$pgdb[8] = "INSERT INTO shablon VALUES('0','Default','64000','64000','ip','100','M','1980-01-01','0')";
$pgdb[9] = "CREATE TABLE timerange ( s_trange_id int NOT NULL AUTO_INCREMENT PRIMARY KEY, s_name varchar(25), s_days varchar(14), s_shour int default '0', s_smin int default '0', s_ehour int default '23', s_emin int default '59')";
$pgdb[10] = "INSERT INTO timerange VALUES('0','Full day','MTWHFAS','0','0','0','0')";
$pgdb[11] = "CREATE TABLE sconfig_time ( s_shablon_id int, s_trange_id int )";
$pgdb[12] = "CREATE TABLE sconfig ( s_shablon_id int, s_redirect_id int )";
$pgdb[13] = "CREATE TABLE redirect ( s_redirect_id int NOT NULL AUTO_INCREMENT PRIMARY KEY, s_name varchar(25), s_type varchar(25) )";
$pgdb[14] = "INSERT INTO redirect SET s_name='Banners', s_type='redir'";
$pgdb[15] = "INSERT INTO redirect SET s_name='Counters', s_type='redir'";
$pgdb[16] = "INSERT INTO redirect SET s_name='Porno', s_type='denied'";
$pgdb[17] = "INSERT INTO redirect SET s_name='Chats', s_type='denied'";
$pgdb[18] = "INSERT INTO redirect SET s_name='Allow1', s_type='allow'";
$pgdb[19] = "INSERT INTO redirect SET s_name='Allow2', s_type='allow'";
$pgdb[20] = "INSERT INTO redirect SET s_name='Porno', s_type='files'";
$pgdb[21] = "INSERT INTO redirect SET s_name='Chats', s_type='files'";
$pgdb[22] = "CREATE TABLE samslog ( s_log_id int NOT NULL AUTO_INCREMENT PRIMARY KEY, s_issuer varchar(50) NOT NULL , s_date date NOT NULL, s_time time NOT NULL, s_value varchar(60) NOT NULL, s_code char(2) )";
$pgdb[23] = "CREATE TABLE sgroup ( s_group_id int NOT NULL AUTO_INCREMENT PRIMARY KEY, s_name varchar(50) )";
$pgdb[24] = "INSERT INTO sgroup SET s_group_id='1', s_name='Admins'";
$pgdb[25] = "INSERT INTO sgroup SET s_group_id='2', s_name='Users'";
$pgdb[26] = "CREATE TABLE reconfig ( s_proxy_id int, s_number int, s_service varchar(15), s_action varchar(10) )";
$pgdb[27] = "CREATE TABLE squiduser ( s_user_id int, s_group_id int, s_shablon_id int, s_nick varchar(50), s_family varchar(50), s_name varchar(50), s_soname varchar(50), s_domain varchar(50), s_quote int NOT NULL default '0', s_size bigint UNSIGNED NOT NULL default '0', s_hit bigint UNSIGNED NOT NULL default '0', s_enabled smallint (1), s_ip char (15), s_passwd varchar(20), s_gauditor smallint, s_autherrorc smallint default '0', s_autherrort varchar(16) default '0' )";
