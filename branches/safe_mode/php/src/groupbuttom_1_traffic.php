...
function GroupTrafficPeriodGB()
{
  ...
  $result=mysql_query("SELECT user,cachesum.domain,sum(cachesum.hit),sum(cachesum.size) as sizeall FROM $SAMSConf->MYSQLDATABASE.cachesum , $SAMSConf->SQUIDCTRLDATABASE.squidusers as tu WHERE tu.nick=cachesum.user && tu.domain=cachesum.domain &&  tu.group=\"$groupname\" && date>=\"$sdate\"&&date<=\"$edate\" group by user order by sizeall desc");
  $count=0;
...
function GroupTrafficPeriod()
{
 ...
 $result=mysql_query("SELECT cachesum.user,cachesum.domain,sum(cachesum.hit),sum(cachesum.size) as sizeall, tu.name, tu.family, cachesum.domain FROM $SAMSConf->MYSQLDATABASE.cachesum, $SAMSConf->SQUIDCTRLDATABASE.squidusers as tu WHERE tu.nick=cachesum.user && tu.domain=cachesum.domain &&  tu.group=\"$groupname\" && date>=\"$sdate\"&&date<=\"$edate\" group by user order by sizeall desc");
  while ($row=mysql_fetch_array($result))
       {
         print("<TR>");
         LTableCell($count,8);
