...
function URLTimePeriodGB()
{
  ...
  $acount=0;
  $query="SELECT HOUR(time) as hour,MINUTE(time) as minute,SUM(size) as size,SUM(hit) as hit FROM $SAMSConf->MYSQLDATABASE.cache WHERE cache.user=\"$username\"&&cache.domain=\"$userdomain\"&&cache.date=\"$sdate\"&&HOUR(time)>=$shou&&HOUR(time)<$ehou GROUP BY hour,minute";
...
//  print("<TH>$userbuttom_4_site_UserSitesPeriod_3");
  ...
  $query="CREATE TEMPORARY TABLE $SAMSConf->MYSQLDATABASE.cache_ SELECT date,time,HOUR(time) as hour,MINUTE(time) as minute,user,domain,trim(leading \"http://\" from substring_index(url,'/',3)) as norm_url FROM $SAMSConf->MYSQLDATABASE.cache WHERE cache.user=\"$username\"&&cache.domain=\"$userdomain\"&&cache.date=\"$sdate\"&&HOUR(time)>=$shou&&HOUR(time)<$ehou ORDER BY hour,minute,url";
  $result=mysql_query("$query");
//  print("query=$query<BR>result = $result");
  
  $query=" SELECT * FROM $SAMSConf->MYSQLDATABASE.cache_  GROUP BY hour,minute,norm_url";
  $result=mysql_query("$query");
  //print("result = $result");
  $cache=0; 
  $counter=0;  
