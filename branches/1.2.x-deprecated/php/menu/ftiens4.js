function gFld2(description, hreference, iconname) 
{ 
  folder = new Folder2(description, hreference, iconname);
  return folder;
} 

function linkFolderHTML2(isTextLink) 
{ 
  var docW2 = "";
  ...
  { 
	if (USEFRAMES)
	  docW2 = docW2 + "<a href='" + this.hreference + "' TARGET=\"tray\" " 
	else
	  docW2 = docW2 + "<a href='" + this.hreference + "' TARGET=_top "
        
    if (isTextLink) {
        docW2 += "id=\"itemTextLink"+this.id+"\" ";
    }

    if (browserVersion > 0) 
      docW2 = docW2 + "onClick='javascript:clickOnFolder(\""+this.getID()+"\")'"
...
function Folder2(folderDescription, hreference, iconname) //constructor 
{ 
  //constant data 
  ...
  this.nodeImg = 0;
  this.isLastNode = 0;
  this.iconSrc = ICONPATH + iconname;
  this.iconSrcClosed = ICONPATH + iconname;
...
// ***************************************************************** 
function Folder(folderDescription, hreference,iconname) //constructor 
{ 
  //constant data 
  this.desc = folderDescription; 
  this.hreference = hreference;
...
function Item(itemDescription, itemLink, target, iconname) // Constructor 
{ 
  // constant data 
...
function gFld(description, hreference, iconname) 
{ 
  folder = new Folder(description, hreference, iconname);
  return folder;
} 

 
function gLnk(optionFlags, description, linkData, iconname) 
{ 
  var fullLink = "";
  var targetFlag = "";
  var target = "";
  ...
  linkItem = new Item(description, protocol+linkData, target, iconname)
  return linkItem 
} 
...
function insFld2(parentFolder, childFolder) 
{ 
