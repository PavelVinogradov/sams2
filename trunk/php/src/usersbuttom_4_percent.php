...
$percent=$all/100;
  ...
  $result=mysql_query("SELECT groups.nick,sum(user_size) as grp_size FROM squidctrl.squidusers,squidctrl.groups,cache_ where cache_.user=squidusers.nick && squidusers.group=groups.name group by groups.nick order by grp_size DESC");
  $count=0;
  while($row=mysql_fetch_array($result))
    {
...
$result=mysql_query("SELECT cache_.user,cache_.domain,cache_.user_size,round(cache_.user_size/$percent,2) as percent,squidusers.name,squidusers.family,cache_.domain from cache_  LEFT JOIN squidctrl.squidusers ON cache_.user=squidusers.nick order by cache_.user_size desc");
$ap=0;
$count=1;
while($row=mysql_fetch_array($result))
  {
