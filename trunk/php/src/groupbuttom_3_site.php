...
function GroupSitesPeriod()
{
  ...
  db_connect("squidlog") or exit();
  mysql_select_db("squidlog");
  $result=mysql_query("CREATE TEMPORARY TABLE cache_ SELECT tc.* FROM squidlog.cache AS tc, squidctrl.squidusers AS tu WHERE tc.user = tu.nick AND tc.domain = tu.domain AND tu.group=\"$groupname\" AND tc.date>=\"$sdate\" AND tc.date<=\"$edate\"");
